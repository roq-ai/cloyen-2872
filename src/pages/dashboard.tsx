...
import { getCustomerAdViews } from 'apiSdk/customer-ad-views';
...
function DashboardPage() {
  ...
  const adCampaignStat = useSWR<PaginatedInterface<AdCampaignInterface>>(
    () => `/ad-campaigns`,
    () => getAdCampaigns({ limit: 1, offset: 0 }),
  );
  ...
  const customerAdViewStat = useSWR<PaginatedInterface<CustomerAdViewInterface>>(
    () => `/customer-ad-views`,
    () => getCustomerAdViews({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Ad Campaigns',
      result: adCampaignStat?.data?.totalCount,
      isLoading: adCampaignStat.isLoading,
    ...
    {
      title: 'Customer Ad Views',
      result: customerAdViewStat?.data?.totalCount,
      isLoading: customerAdViewStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
