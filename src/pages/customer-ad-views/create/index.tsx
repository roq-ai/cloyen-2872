...
import { createCustomerAdView } from 'apiSdk/customer-ad-views';
...
function CustomerAdViewCreatePage() {
  ...
  const handleSubmit = async (values: CustomerAdViewInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createCustomerAdView(values);
      resetForm();
  ...
  const formik = useFormik<CustomerAdViewInterface>({
    initialValues: {
      view_date: new Date(new Date().toDateString()),
      customer_id: (router.query.customer_id as string) ?? null,
      ad_campaign_id: (router.query.ad_campaign_id as string) ?? null,
    },
